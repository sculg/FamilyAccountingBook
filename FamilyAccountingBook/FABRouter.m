...
static NSString * const FAB_ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *specialCharacters = @"/?&.";

NSString *const FABRouterParameterURL = @"FABRouterParameterURL";
NSString *const FABRouterParameterCompletion = @"FABRouterParameterCompletion";
NSString *const FABRouterParameterUserInfo = @"FABRouterParameterUserInfo";
...
@implementation FABRouter

+ (instancetype)sharedInstance
{
    static FABRouter *instance = nil;
...
+ (void)registerURLPattern:(NSString *)URLPattern toHandler:(FABRouterHandler)handler
{
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        FABRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[FABRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[FABRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
{
    FABRouter *router = [FABRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL matchExactly:NO];
    FABRouterObjectHandler handler = parameters[@"block"];
...
+ (void)registerURLPattern:(NSString *)URLPattern toObjectHandler:(FABRouterObjectHandler)handler
{
    [[self sharedInstance] addURLPattern:URLPattern andObjectHandler:handler];
}

- (void)addURLPattern:(NSString *)URLPattern andHandler:(FABRouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(FABRouterObjectHandler)handler
{
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url matchExactly:(BOOL)exactly
{
    ...
    parameters[FABRouterParameterURL] = url;
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:FAB_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if (!URL.length) {
            [pathComponents addObject:FAB_ROUTER_WILDCARD_CHARACTER];
        }
    }
