...
@interface FABInquiryPeriodEntity()
/** 时间段sql */
@property (nonatomic, copy) NSString *inquiryPeriodSql;

@end

@implementation FABInquiryPeriodEntity



-(NSInteger)periodRecordNum{
    LKDBHelper* globalHelper = [FABAccountingRecentRecordEntity getUsingLKDBHelper];
    NSArray *myRecordArray = [globalHelper search:[FABAccountingRecentRecordEntity class] where:_inquiryPeriodSql orderBy:nil offset:0 count:1000];
...
-(double)incomeSum{
    ...
    double sum = 0;
    _inquiryPeriodSql = [NSString stringWithFormat:@"recordTime >= '%@ 00:00:00' and recordTime <= '%@ 23:59:59'",_startDate,_endDate];
    myRecordArray = [globalHelper search:[FABAccountingRecentRecordEntity class] where:[NSString stringWithFormat:@"%@ and accountingType = 0",_inquiryPeriodSql] orderBy:@"recordTime desc" offset:0 count:1000];
...
-(double)expenditureSum{
    ...
    double sum = 0;
    _inquiryPeriodSql = [NSString stringWithFormat:@"recordTime >= '%@ 00:00:00' and recordTime <= '%@ 23:59:59'",_startDate,_endDate];
    myRecordArray = [globalHelper search:[FABAccountingRecentRecordEntity class] where:[NSString stringWithFormat:@"%@ and accountingType = 1",_inquiryPeriodSql] orderBy:@"recordTime desc" offset:0 count:1000];
...
//结余总金额
-(double)cashSurplus{
    return self.incomeSum + self.expenditureSum;
}
...
-(double)inquireIncomeSumWithType:(NSInteger)typeId{
    ...
    double sum = 0;
    _inquiryPeriodSql = [NSString stringWithFormat:@"recordTime >= '%@ 00:00:00' and recordTime <= '%@ 23:59:59'",_startDate,_endDate];
    myRecordArray = [globalHelper search:[FABAccountingRecentRecordEntity class] where:[NSString stringWithFormat:@"%@ and incomeClassificationType = %d",_inquiryPeriodSql,(int)typeId] orderBy:@"recordTime desc" offset:0 count:1000];
    for (FABAccountingRecentRecordEntity *entity in myRecordArray) {
        sum += [entity.recordMoney doubleValue];
    }
    return sum;
...
-(double)inquireEpenditureSumWithType:(NSInteger)typeId{
    ...
    myRecordArray = [globalHelper search:[FABAccountingRecentRecordEntity class] where:[NSString stringWithFormat:@"%@ and ependitureClassificationType = %d",_inquiryPeriodSql,(int)typeId] orderBy:@"recordTime desc" offset:0 count:1000];
    for (FABAccountingRecentRecordEntity *entity in myRecordArray) {
        sum += [entity.recordMoney doubleValue];
    }
    return sum;
...
-(double)inquireEpenditureSumWithPayType:(NSInteger)typeId{
    ...
    myRecordArray = [globalHelper search:[FABAccountingRecentRecordEntity class] where:[NSString stringWithFormat:@"%@ and payType = %d and accountingType = 1",_inquiryPeriodSql,(int)typeId] orderBy:@"recordTime desc" offset:0 count:1000];
    for (FABAccountingRecentRecordEntity *entity in myRecordArray) {
        sum += [entity.recordMoney doubleValue];
    }
    return sum;
...
-(double)inquireEpenditureSumWithObjectType:(NSInteger)typeId{
    ...
    myRecordArray = [globalHelper search:[FABAccountingRecentRecordEntity class] where:[NSString stringWithFormat:@"%@ and expenditureObjectType = %d and accountingType = 1",_inquiryPeriodSql,(int)typeId] orderBy:@"recordTime desc" offset:0 count:1000];
    for (FABAccountingRecentRecordEntity *entity in myRecordArray) {
        sum += [entity.recordMoney doubleValue];
    }
    return sum;
