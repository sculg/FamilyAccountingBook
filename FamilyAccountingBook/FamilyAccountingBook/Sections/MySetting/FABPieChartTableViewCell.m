...
- (id)initWithCellIdentifier:(NSString *)cellId {
    ...
    if (self) {
        ...
        [self.contentView addSubview:self.myChartView];
    }
...
- (void)layoutSubviews {
    ...
    NSString *myValue = [NSString stringWithFormat:@"%@%@",[self.sum stringValue],kCashUnit];
...
- (NSString * _Nonnull)stringForValue:(double)value entry:(ChartDataEntry * _Nonnull)entry dataSetIndex:(NSInteger)dataSetIndex viewPortHandler:(ChartViewPortHandler * _Nullable)viewPortHandler SWIFT_WARN_UNUSED_RESULT{
    ...
    return [NSString stringWithFormat:@"%1.f%% (%ld %@)",value,(NSInteger)entry.y,kCashUnit];
