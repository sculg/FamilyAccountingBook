...
+ (NSString*)weekdayStringFromDate:(NSString*)inputDateStr {
    NSDate *date = [NSDate dateFromString:inputDateStr withFormat:@"yyyy-MM-dd"];
    NSArray *weekdays = [NSArray arrayWithObjects: [NSNull null], NSLocalizedString(@"Sun",nil), NSLocalizedString(@"Mon",nil), NSLocalizedString(@"Tues",nil), NSLocalizedString(@"Wed",nil), NSLocalizedString(@"Thur",nil), NSLocalizedString(@"Fri",nil), NSLocalizedString(@"Sat",nil), nil];
...
+ (NSString *)convertStr_yyyy_MM_ddToDisplay:(NSString *)str_yyyy_MM_dd{
    ...
    if ([date year] != [[NSDate date] year]) {
        displayStr = [date stringWithFormat:@"yy-MM"];
    }else{
        switch ([date leftDayCount]) {
            ...
            case 1:
                ...
                break;
            case 0:
                displayStr = NSLocalizedString(@"Today",nil);
                break;
            case -1:
                displayStr = NSLocalizedString(@"Yesterday",nil);
                break;
//            case -2:
//                displayStr = @"前  天";
//                break;
            default:
                displayStr = [date stringWithFormat:@"MM-dd"];
                break;
        }
    }
    return displayStr;
